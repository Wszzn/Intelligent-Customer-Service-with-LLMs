...
def split_by_title(text: str, base_dir: str = "", max_chunk_size: int = 1024) -> List[Dict]:
    ...
    for line in text.split('\n'):
        ...
        if title_match:
            ...
            # 开始新的chunk
            current_chunk = []
            level = len(title_match.group(1))  # 获取标题级别（#的数量）
            title = title_match.group(2)
            
            # 更新标题层级
            while header_stack and header_stack[-1][0] >= level:
                header_stack.pop()  # 移除同级或更低级的标题
            ...
            # 更新当前标题信息
            current_headers = {}
            for i, (hlevel, htitle) in enumerate(header_stack, 1):
                current_headers[f"Header {i}"] = htitle
            
            # 如果缺少上级标题，添加默认值
            for i in range(1, level):
                if f"Header {i}" not in current_headers:
                    current_headers[f"Header {i}"] = f"未命名章节{i}"
        else:
    ...
    for chunk in chunks:
        if len(chunk["text"]) > max_chunk_size:
            split_chunks = split_text_by_size(chunk["text"], max_chunk_size)
...
def split_text_by_size(text: str, chunk_size: int = 1024, overlap: int = 200) -> List[str]:
    """
